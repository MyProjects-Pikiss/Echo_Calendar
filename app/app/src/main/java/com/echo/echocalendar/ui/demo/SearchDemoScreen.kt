...
fun SearchDemoScreen(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        androidx.compose.material3.OutlinedTextField(
            modifier = Modifier.fillMaxWidth(),
            value = searchViewModel.labelFilters.joinToString(", "),
            onValueChange = searchViewModel::onLabelFiltersChange,
            label = { Text("라벨 필터 (최대 5개, 쉼표 구분)") },
            singleLine = true
        )

        Spacer(modifier = Modifier.height(8.dp))
        Text(
            text = "필터를 변경하면 자동으로 재검색됩니다.",
        ...
        if (hasFilters) {
            ...
            FlowRow(
                ...
                searchViewModel.labelFilters.forEach { label ->
                    FilterChip(
                        selected = true,
                        onClick = { searchViewModel.removeLabelFilter(label) },
                        label = { Text("라벨: $label ✕") }
                    )
                }
            }
        ...
        Spacer(modifier = Modifier.height(12.dp))
        Text(
            text = if (searchViewModel.hasSearched) {
                "검색 결과 (${searchViewModel.results.size}건)"
            } else {
                "검색 결과"
            },
